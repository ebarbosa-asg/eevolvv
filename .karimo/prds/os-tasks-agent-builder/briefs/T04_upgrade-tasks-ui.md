...
This task is part of **Wave 2** — it depends on T02 (which adds the new `service_tasks` columns). T05 (agent builder page) and T06 (wire build links) come after.
...
- Note: `taskForm.estimated_hrs` is kept as a string in state (empty string for blank). `parseFloat` converts it on submit. `editTaskForm` uses `Partial<Task>` which types `estimated_hrs` as `number | null`. These are separate state objects — do not assign one to the other.
...
|------|------------------|------------------------|
| T02 | Adds `agent_id`, `category`, `assignee`, `estimated_hrs`, `blocked_reason` columns to `service_tasks` | Call Supabase `list_tables verbose: true` and confirm `agent_id`, `category`, `estimated_hrs`, `blocked_reason`, `assignee` columns exist on `service_tasks` |

**Before starting:** Verify T02 is complete by confirming the 5 new `service_tasks` columns exist in the live DB. The TypeScript types in this task are valid regardless, but the UI runtime behavior depends on these columns being present.
...
*PRD: os-tasks-agent-builder | Task: T04 | Wave: 2*
