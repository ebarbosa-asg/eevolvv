...
This task is part of **Wave 1** — it can run in parallel with T04 and T06 since it creates all-new files.
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            {stage.promoteLabel && isActive && (
              <button
                ...
                style={{ ...MONO, fontSize: '11px', color: 'var(--paper)', background: stage.borderColor === '#4ade80' ? '#4ade80' : 'none', border: `1px solid ${stage.borderColor}`, padding: '4px 10px', cursor: 'pointer', borderRadius: '2px', color: stage.borderColor }}
              >
                {stage.promoteLabel}
              </button>
            )}
...
- [ ] `AgentFull` type includes all new DB fields: `trigger_type`, `trigger_config`, `instructions`, `config`, `version`
...
None — T05 creates all-new files and can start immediately. The DB migrations for `agents` table new columns should ideally be applied before deployment, but TypeScript types can be written regardless.
...
*PRD: os-tasks-agent-builder | Task: T05 | Wave: 1*
