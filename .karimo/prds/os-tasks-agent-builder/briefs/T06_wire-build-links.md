...
`ClientWorkspace.tsx` is the per-client page at `/os/clients/[id]`. It has already been modified by T04 (Wave 1) to add the extended `Task` type including `agent_id`, and the task rows already render an agent link pill as part of T04's scope.

T05 (Wave 1) created the agent builder route at `/os/clients/[id]/agents/[agentId]`.

This task (Wave 2) wires the final navigation layer: the `→ build` link on agent cards in § A AGENTS. It runs after T04 and T05 are merged.
...
This task is part of **Wave 2** — depends on T04 (extended Task type + agents state in scope) and T05 (the builder route must exist for links to be correct).
...
T04 also modified this file (Wave 1). This task (T06, Wave 2) runs after T04 is merged, so the file already has:
...
*PRD: os-tasks-agent-builder | Task: T06 | Wave: 2*
