...
Write `docs/seed-build-sop.md` — the technician's complete playbook for delivering a Seed-tier build (landing page + 1 automation workflow) within the 72-hour SLA. This is a documentation-only task; no code is written. The SOP must be specific enough that a new technician can complete a Seed build without asking E for guidance.
...
- `app/onboard/[token]/page.tsx` exists — the SOP references the intake form fields (T09 complete)
- `app/os/builds/page.tsx` will exist after T16 — reference it as the build queue UI

**Context from PRD research summary:**
- Seed is ~85% automatable — technician handles QA approval and Vercel deploy
- 5 Seed build types: service business (plumber, cleaner, trainer), local business (restaurant, shop), product waitlist, portfolio, simple e-commerce
- Claude Code + v0.dev is the intended toolchain for Seed builds
- QA: Lighthouse score ≥90, mobile responsive, no broken links, form submission tested
- 72-hour SLA starts from when `builds.status` transitions to `in_progress` (technician claims the build)

**Docs folder:** `docs/` already contains `eevolvv-service-agreement.docx` and strategy documents. Create `docs/seed-build-sop.md` in the same location.
...
- `docs/seed-build-sop.md` — Full technician SOP
...
Write `docs/seed-build-sop.md` with the following structure and content:
...
- [ ] `docs/seed-build-sop.md` created with all 9 steps
- [ ] All 5 Claude Code prompt templates present (A through E)
- [ ] Build type selection table present
- [ ] QA checklist is complete (9 checkboxes minimum)
- [ ] Automation workflow options table present
- [ ] Vercel deploy steps included
- [ ] Escalation procedure documented
- [ ] 72-hour SLA timing explicitly defined (starts at `builds.status = in_progress`)
- [ ] References to OS build queue (`/os/builds`) and client portal (`/client/[token]`)
...
| Output | Consumed by |
|--------|------------|
| `docs/seed-build-sop.md` | Technician (human) — no downstream code tasks |
...
- Do not create API routes or React components
- Do not modify any existing files
- Do not add pricing or sales copy — this is internal operations documentation
- Do not include client-specific data — templates only
