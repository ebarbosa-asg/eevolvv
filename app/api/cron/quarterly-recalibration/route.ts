...
export async function GET(req: NextRequest) {
  ...
  const checkInId = Sentry.captureCheckIn(
    { monitorSlug: 'cron-recalibration', status: 'in_progress' },
    {
      schedule: { type: 'crontab', value: '0 6 * * 3' },
      checkinMargin: 5,
      maxRuntime: 10,
  ...
  try {
    ...
    if (!subscriptions?.length) {
      Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-recalibration', status: 'ok' })
    ...
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-recalibration', status: 'error' })
    throw err
  }
}
