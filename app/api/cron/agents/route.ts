...
export async function GET(req: NextRequest) {
  ...
  const checkInId = Sentry.captureCheckIn(
    { monitorSlug: 'cron-agents', status: 'in_progress' },
    {
  ...
  try {
    ...
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-agents', status: 'ok' })
    return NextResponse.json({ fired, skipped, total: agents.length })
  } catch (err) {
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-agents', status: 'error' })
    throw err
  }
}
