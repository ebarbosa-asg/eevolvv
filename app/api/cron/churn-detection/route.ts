...
export async function GET(req: NextRequest) {
  ...
  const checkInId = Sentry.captureCheckIn(
    { monitorSlug: 'cron-churn-detection', status: 'in_progress' },
    {
      schedule: { type: 'crontab', value: '0 7 * * 1' },
      checkinMargin: 5,
      maxRuntime: 10,
  ...
  try {
    ...
    if (activeClientIds.length === 0) {
      Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-churn-detection', status: 'ok' })
    ...
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-churn-detection', status: 'error' })
    throw err
  }
}
