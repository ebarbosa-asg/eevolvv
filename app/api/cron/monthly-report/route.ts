...
export async function GET(req: NextRequest) {
  ...
  const checkInId = Sentry.captureCheckIn(
    { monitorSlug: 'cron-monthly-report', status: 'in_progress' },
    {
  ...
  try {
    ...
    if (!subscriptions?.length) {
      Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-monthly-report', status: 'ok' })
    ...
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-monthly-report', status: 'error' })
    throw err
  }
}
