...
export async function GET(req: NextRequest) {
  ...
  const checkInId = Sentry.captureCheckIn(
    { monitorSlug: 'cron-followup', status: 'in_progress' },
    {
  ...
  try {
    ...
    if (!submissions?.length) {
      Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-followup', status: 'ok' })
    ...
    Sentry.captureCheckIn({ checkInId, monitorSlug: 'cron-followup', status: 'error' })
    throw err
  }
}
